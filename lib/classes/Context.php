        ...
        if ($context instanceof Course) {
            ...
            if (!$GLOBALS['perm']->get_studip_perm($context->id)) {
                if ($context->lesezugriff > 0 || !Config::get()->ENABLE_FREE_ACCESS) {
                    ...
                    if (!$GLOBALS['perm']->get_studip_perm($context->id && !match_route('dispatch.php/course/details'))) {
                        PageLayout::postWarning(_('Sie sind noch nich eingetragen. Bitte prüfen Sie die Anmelderegeln!'));
            ...
            if ($context->aux_lock_rule_forced
            ...
            if (!$GLOBALS['perm']->get_studip_perm(self::getId()) && !match_route('dispatch.php/institute/overview')) {
                PageLayout::postWarning(_('Sie sind zu dieser Einrichtung nicht zugeordnet!'));
                $url = URLHelper::getURL(
